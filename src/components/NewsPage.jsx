...
import { useNews } from "../hooks/useTennisData";
...
function NewsCard({ article, isFeatured, onRead }) {
  const [bookmarked, setBookmarked] = useState(false);

  if (isFeatured) {
    return (
      <div
        className="card col-span-full bg-court-green text-white relative overflow-hidden border-0 cursor-pointer hover:scale-[1.01] transition-transform"
        onClick={onRead}
      >
        <NetPattern />
        <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              ...
              <span className="text-white/40 text-xs">{article.published_at?.slice(0, 10)}</span>
            </div>
            ...
            <div className="flex items-center gap-3">
              <span className="text-white/50 text-sm">📖 {article.read_time_min} min de lecture</span>
              <button
                onClick={e => { e.stopPropagation(); onRead(); }}
                className="ml-auto bg-court-yellow text-court-green font-black px-5 py-2 rounded-xl text-sm hover:bg-white transition-colors"
              >
                Lire l'article →
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                  bookmarked ? "bg-white text-court-green" : "bg-white/20 text-white hover:bg-white/30"
                }`}
              >
  ...
  return (
    <div
      className="card hover:shadow-xl transition-all duration-200 hover:-translate-y-0.5 group cursor-pointer"
      onClick={onRead}
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-court-blue text-xs font-bold group-hover:underline">Lire →</span>
              <button
                onClick={e => { e.stopPropagation(); setBookmarked(!bookmarked); }}
...
export default function NewsPage() {
  ...
  const hotCount = news.filter(n => n.is_hot).length;

  return (
    <div className="space-y-6 animate-fade-in">
      {selectedArticle && (
        <ArticleModal article={selectedArticle} type="news" onClose={() => setSelectedArticle(null)} />
      )}
      ...
      <div className="relative rounded-3xl overflow-hidden bg-court-clay p-8 text-white">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 opacity-70">{hotCount} articles chauds · Clique pour lire l'article complet</p>
          </div>
      ...
      <div className="rounded-2xl border-2 border-dashed border-court-blue/30 p-5 flex items-center gap-4 bg-court-blue/5">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-sm">Connecte ta montre pour recevoir les résultats en direct sur ton poignet !</p>
        </div>
      </div>
    </div>
  );
